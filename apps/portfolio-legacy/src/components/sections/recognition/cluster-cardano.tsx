...
export const ClusterCardano = memo(
  forwardRef<HTMLDivElement, ClusterCardanoProps>(
    ({ images, onImageClick, className }, ref) => {
      // Destructure className
      return (
        <div
          ref={ref}
          className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex gap-8 w-[60vw] md:w-[40vw] pointer-events-none [&_button]:pointer-events-[inherit] will-change-transform ${
            className || ''
          }`}
